...
export const Modal: React.FC<ModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl shadow-slate-400/20 w-full max-w-sm overflow-hidden animate-in zoom-in-[0.98] duration-300 border border-white/50">
        <div className="p-8 text-center">
          <div className={`w-16 h-16 ${iconBgColor} rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm ring-4 ring-white`}>
            {icon}
          </div>
        ...
        <div className="flex border-t border-slate-100 p-4 gap-3 bg-slate-50/50">
          {showCancel && (
            <button 
              onClick={onClose}
              className="flex-1 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-white hover:text-slate-800 rounded-xl transition-all border border-transparent hover:border-slate-200"
            >
              Cancel
            </button>
          )}
