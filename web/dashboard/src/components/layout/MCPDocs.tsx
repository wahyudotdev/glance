      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-800">Model Context Protocol</h3>
              <p className="text-xs text-slate-500">Connect AI Agents to this Proxy</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white rounded-lg text-slate-400">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <section className="space-y-4">
            ...
            <p className="text-xs text-slate-600">Add this to your <code>claude_desktop_config.json</code>:</p>
    ...
    "agent-proxy": {
      ...
      "args": ["-y", "@modelcontextprotocol/server-sse", "${mcpUrl}"]
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={tool.name} className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                  <code className="text-[10px] font-bold text-indigo-600">{tool.name}</code>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-center">
          <button 
            onClick={onClose}
            className="px-8 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all"
          >
            Close Documentation
          </button>
        </div>
